...
const {
  ...
  const { deployDiamond } = require('../scripts/deploySBT.ts')
  
  const { assert } = require('chai')
  
  describe('SBTFacetTest', async function () {
    let diamondAddress
    let diamondCutFacet
    ...
    it('should test SBT call => setConstructor ', async () => {
        await sbtFacet.setConstructor(
            "NirVANA_name",
            "NirVANA_symbol",
            "NirVANA_desc"
        );
        assert.equal(await sbtFacet.name(), "NirVANA_name");
        assert.equal(await sbtFacet.symbol(), "NirVANA_symbol");
        assert.equal(await sbtFacet.desc(), "NirVANA_desc");
        const accounts = await ethers.getSigners()
        // console.log(accounts[0].address);
        // console.log(await sbtFacet.owner())
        assert.equal(await sbtFacet.getOwner(), accounts[0].address);
    })

    it('should test SBT call => mint ', async () => {
        const accounts = await ethers.getSigners()
        for(let i = 0;i < 5;++i) {
    ...
    it('should add DAO functions', async () => {
        ...
        addresses.push(daoFacet.address)
        const alreadyFacetAddresses = await diamondLoupeFacet.facetAddresses()
        let selectors = getSelectors(daoFacet)
        for(let i = 0; i < alreadyFacetAddresses.length; ++i) {
            // console.log(selectors)
            // console.log('///////////')
            selectors = getRemovedSelectorsBySelector(
                selectors, 
                await diamondLoupeFacet.facetFunctionSelectors(alreadyFacetAddresses[i])
            )
        }
        ...
        result = await diamondLoupeFacet.facetFunctionSelectors(daoFacet.address)
    ...
    //   assert.sameMembers(result, selectors)
    // //   selectors = getSelectors(sbtFacet)
